    ...
    <div class="lot-item__content">
      ...
      <div class="lot-item__right">
        ...
        <div class="lot-item__state">
          <div class="lot-item__timer timer <?php if (($lot['date_end'] ?? 0) < time()): ?>timer--end<?php endif; ?>">
            <?=countRemainingTime($lot['date_end'] ?? 0); ?>
          </div>
          <div class="lot-item__cost-state">
            <div class="lot-item__rate">
              <span class="lot-item__amount">Текущая цена</span>
              <span class="lot-item__cost"><?=formatSum(htmlspecialchars($lot['start_price'] ?? 0)); ?></span>
            </div>
            <div class="lot-item__min-cost">
              Мин. ставка <span><?=formatSum(htmlspecialchars($lot['step'] ?? 0), '&nbsp;р'); ?></span>
            </div>
          </div>
          <?php if ($canAddBet): ?>
        ...
        <?php if ($bets): ?>
          ...
          <?php foreach($bets as $bet):?>
            <tr class="history__item">
              ...
              <td class="history__price"><?=formatSum(htmlspecialchars($bet['price'] ?? 0), '&nbsp;р'); ?></td>
              <td class="history__time"><?=formatTime(htmlspecialchars($bet['date_add'] ?? 0)); ?></td>
            </tr>
          <?php endforeach; ?>
          </table>
          <?php else: ?>
